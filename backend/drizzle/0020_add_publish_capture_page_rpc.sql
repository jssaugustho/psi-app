-- Procedure atômica PL/pgSQL para publicação de páginas de captação (Rascunho -> Publicado)
...
BEGIN
  ...
  ELSE
    v_new_canvas := v_page.canvas_data;
  END IF;
  ...
  ELSE
    v_new_site_config := v_page.site_config;
  END IF;

  -- Garante que site_config contenha canvas_data atualizado
  IF v_new_site_config IS NULL THEN
    v_new_site_config := jsonb_build_object('canvas_data', v_new_canvas);
  ELSE
  ...
  SET
    canvas_data = COALESCE(v_new_canvas, canvas_data),
