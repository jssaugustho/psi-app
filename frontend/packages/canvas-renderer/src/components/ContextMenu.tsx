...
export function ContextMenu({
  ...
  const adjustedX = menuState.x + menuWidth > screenWidth ? Math.max(10, screenWidth - menuWidth - 16) : Math.max(10, menuState.x);
  const adjustedY = menuState.y + menuHeight > screenHeight ? Math.max(10, screenHeight - menuHeight - 16) : Math.max(10, menuState.y);

  return (
    <div
      className="fixed z-[9999] w-48 py-1 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl shadow-2xl backdrop-blur-md text-xs font-sans select-none animate-in fade-in zoom-in-95 duration-100"
      style={{
        left: `${adjustedX}px`,
        top: `${adjustedY}px`,
      }}
