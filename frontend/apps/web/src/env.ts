...
const envSchema = z.object({
  ...
  NEXT_PUBLIC_LANDING_BASE_URL: z.string().optional(),
});

const _env = envSchema.safeParse({
  NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL,
  NEXT_PUBLIC_BASE_DOMAIN: process.env.NEXT_PUBLIC_BASE_DOMAIN,
  NEXT_PUBLIC_LANDING_BASE_URL: process.env.NEXT_PUBLIC_LANDING_BASE_URL,
});

if (!_env.success) {
  console.error('❌ ERRO CRÍTICO DE CONFIGURAÇÃO NO FRONTEND (WEB):', _env.error.format());
}
