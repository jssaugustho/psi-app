...
import { getThemeColors, extractCanvasColors, extractCanvasGradients, getTextFallbackColor } from '../../utils/colorHelpers';
...
export function GlobalColorPicker({
  ...
  const getFallbackColor = (): string => {
    ...
    if (l.includes('título') || l.includes('heading') || l.includes('h1') || l.includes('h2') || l.includes('h3')) {
      return getTextFallbackColor(themeColors.siteBg, 'heading');
    }
    if (l.includes('etiqueta') || l.includes('label') || l.includes('badge') || l.includes('subtítulo') || l.includes('subtext')) {
      return getTextFallbackColor(themeColors.siteBg, 'label');
    }
    if (l.includes('texto') || l.includes('text') || l.includes('parágrafo') || l.includes('corpo') || l.includes('descrição') || l.includes('rótulo')) {
      return getTextFallbackColor(themeColors.siteBg, 'body');
    }
  ...
  // Estados dos Parâmetros do Modo Gradiente
  ...
  // Manter estado dos pontos de gradiente sincronizado quando a cor do elemento muda externamente
  useEffect(() => {
  ...
  // Swatch visual — mostra a prévia da cor (mesmo quando herdada)
  ...
  return (
    <div className={`space-y-1 relative ${className}`} ref={containerRef}>
      ...
      {/* Trigger Bar — sem input de texto */}
      <button
        type="button"
