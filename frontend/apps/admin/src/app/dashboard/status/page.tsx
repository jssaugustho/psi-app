...
export default function StatusPage() {
  ...
  }, [user, loading, router, range, loadStatusHistory]);
  ...
  const menuItems = [
    ...
    { label: 'Status do App', href: '/dashboard/status', icon: <StatusIcon />, active: true },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ color: 'var(--brand-text-color)' }}>
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: 'color-mix(in srgb, var(--brand-gradient-start) 30%, transparent)', borderTopColor: 'var(--brand-gradient-start)' }} />
          <p className="text-sm" style={{ opacity: 0.6 }}>Carregando dados de status...</p>
        </div>
      </div>
    );
  }
